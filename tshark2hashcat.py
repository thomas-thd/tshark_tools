...
Protocoles détectés automatiquement :
  ...
  - APOP (POP3)        ... digest = MD5(challenge + password)     -> hashcat -m 20
  - Identifiants clairs... FTP/Telnet USER/PASS, HTTP Basic,
                             SMTP/IMAP/POP AUTH PLAIN & LOGIN     -> credentials.txt

NOTE : hashcat n'a PAS de mode OSPF natif, et l'OSPF de ta capture utilise de
surcroît le crypto-auth LLS (RFC 4813) : pas cassable tel quel par hashcat.
...
def sibling_etype(parent, layers):
    ...
    for k in ("kerberos.PA_ENC_TIMESTAMP_etype", "kerberos.etype"):
        e = first_str(parent.get(k))
        if e and str(e).isdigit():
            return int(e)
    e = find_str(layers, ("kerberos.etype", "kerberos.PA_ENC_TIMESTAMP_etype"))
...
# --------------------------------------------------------------------------- #
def is_hex(s, length=None, minlen=0, maxlen=None):
...
def validate(mode, line):
    ...
    if mode == 5600:                       # user::domain:chal:NTProofStr:blob
        return (p and len(p) == 6 and 1 <= len(p[0]) <= 60 and len(p[1]) == 0
                and len(p[2]) <= 45
                and is_hex(p[3], 16) and is_hex(p[4], 32)
                and is_hex(p[5], minlen=2, maxlen=1024))
    if mode == 5500:                       # user::domain:chal:LM(24o):NT(24o)
        return (p and len(p) == 6 and 1 <= len(p[0]) <= 60 and len(p[1]) == 0
                and len(p[2]) <= 45
                and is_hex(p[3], 16) and is_hex(p[4], 48) and is_hex(p[5], 48))
...
# --------------------------------------------------------------------------- #
# APOP (POP3) -> hashcat -m 20  (md5($salt.$pass))
# --------------------------------------------------------------------------- #
def apop_scan(raw, missing):
    """Authentification APOP : digest = MD5(challenge + password).
    Format hashcat -m 20 : <md5>:<challenge>. La bannière '+OK ... <ts>' fournit
    le challenge ; la commande 'APOP user digest' le digest."""
    ...
    for pos, kind, a, b in events:
        if kind == "chal":
            ...
            user, digest = a, b
            if current:
                res.append((20, f"{digest}:{current}"))
            else:
                missing.append(f"APOP ({user}): challenge introuvable (pas de bannière '<...>') -> ignoré")
    return res


# --------------------------------------------------------------------------- #
# Identifiants en clair (FTP/Telnet/HTTP/IMAP/POP/SMTP) -> rapport credentials.txt
# --------------------------------------------------------------------------- #
def _b64d(s):
    try:
...
def creds_scan(raw):
    """Extrait les identifiants transmis en clair. Retourne [(proto, user, pass)]."""
    text = raw.decode("latin-1", "replace")
    found = []
    ...
    for pos, kind, val in events:
        if kind == "u":
            ...
            found.append(("FTP/Telnet USER/PASS", u, val))
            u = None
    ...
    for m in re.finditer(r"(?i)AUTH\s+PLAIN\s+([A-Za-z0-9+/=]+)", text):
        ...
        if rb is None:
            continue
        parts = rb.split("\x00")
        if len(parts) >= 3:      # authzid\0authcid\0passwd
            found.append(("AUTH PLAIN", parts[-2], parts[-1]))
        elif len(parts) == 2:
            found.append(("AUTH PLAIN", parts[0], parts[1]))

    # --- AUTH LOGIN : b64(user)\r\nb64(pass) ---
    for m in re.finditer(r"(?i)AUTH\s+LOGIN\s*\r?\n\s*([A-Za-z0-9+/=]+)\s*\r?\n\s*([A-Za-z0-9+/=]+)", text):
        us, ps = _b64d(m.group(1)), _b64d(m.group(2))
        if us is not None and ps is not None:
            found.append(("AUTH LOGIN", us, ps))

    # --- HTTP form (GET/POST) user=...&pass=... ---
    for m in re.finditer(r"(?i)(?:user(?:name)?|login)\s*=\s*([^&\s]+)\s*&\s*(?:pass(?:word)?|pwd)\s*=\s*([^&\s]+)",
                         text):
        found.append(("HTTP form", m.group(1), m.group(2)))
...
def find_tshark(cli_path):
    ...
    for c in ([cli_path] + TSHARK_CANDIDATES if cli_path else TSHARK_CANDIDATES):
        if c and (os.path.isfile(c) or which(c)):
            return which(c) or c
    return None
...
def main():
    ...
    if args.pcap.lower().endswith(".json"):
        ...
        with open(args.pcap, encoding="utf-8", errors="replace") as f:
            packets = json.load(f)
    else:
    ...
    for idx, pkt in enumerate(packets, 1):
        ...
        if ntlm:
            ...
            if mtype == 2:                            # CHALLENGE (srv->cli)
                ...
                if not chal:                          # appairer au challenge
                    ...
                    chal = next((c for f, t, c in reversed(ntlm_chals) if want and t == want),
        ...
        # ---- octets bruts (-x) accumulés pour scans globaux (NTLMSSP, APOP, creds) ----
        raw_all.extend(
            bytes.fromhex(norm_hex(v[0]) or "")
            for k, v in walk(layers) if k.endswith("_raw") and isinstance(v, list) and v)

    # ---- scans bruts GLOBAUX (dans l'ordre des trames) ----
    raw_blob = b"".join(raw_all)

    # NTLM non disséqué (signature binaire + base64 HTTP/IMAP/SMTP)
    ...
    # APOP (POP3) -> hashcat -m 20
    if raw_blob and re.search(rb"\bAPOP\s+\S+\s+[0-9a-fA-F]{32}\b", raw_blob):
        for mode, line in apop_scan(raw_blob, missing):
            push(mode, line, "APOP")

    # Identifiants en clair -> rapport credentials.txt
    credentials = creds_scan(raw_blob) if raw_blob else []
    if credentials:
    ...
    if credentials:
        cred_fn = os.path.splitext(args.output)[0] + "_credentials.txt"
        with open(cred_fn, "w", encoding="utf-8") as f:
            for proto, u, p in credentials:
                f.write(f"{proto}\tuser={u}\tpass={p}\n")
        print(f"\n[+] {len(credentials)} identifiants en clair -> {cred_fn}")
        print("=== IDENTIFIANTS EN CLAIR ===")
        for proto, u, p in credentials:
            print(f"    {proto:<22} {u} : {p}")
